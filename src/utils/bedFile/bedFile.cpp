...
void BedFile::loadBedFileIntoVector() {
	...
	//unsigned int reserve_time = out(t_start);

	//t_start = in();
    Open();
	//unsigned int open_time = out(t_start);

	//unsigned int get_next_bed_time = 0;
	//unsigned int vector_push_time = 0;
	//t_start = in();
    while ((bedStatus = this->GetNextBed(bedEntry, lineNum)) != BED_INVALID) {
		//get_next_bed_time += out(t_start);
	...
	//t_start = in();
    Close();
	//unsigned int close_time = out(t_start);

	//unsigned int total_time = reserve_time +
							  //open_time +
							  //get_next_bed_time +
							  //vector_push_time +
							  //close_time;
	//cerr <<
		//"\t" << "reserve_time:" <<
		//reserve_time << "\t" <<
		//(double)reserve_time/total_time << endl <<
		//"\t" << "open_time:" <<
		//open_time << "\t" <<
		//(double)open_time/total_time << endl <<
		//"\t" << "get_next_bed_time:" <<
		//get_next_bed_time << "\t" <<
		//(double)get_next_bed_time/total_time << endl <<
		//"\t" << "vector_push_time:" <<
		//vector_push_time << "\t" <<
		//(double)vector_push_time/total_time << endl <<
		//"\t" << "close_time:" <<
		//close_time << "\t" <<
